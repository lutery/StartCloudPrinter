...
public class CRC16CCITTVerify implements IVerify {
    ...
    public boolean verifyContent(byte[] verifyCode, byte[] verifyContent) throws VerifyException {
        ...
        byte[] crc_ccitt = this.generateVerifyCode(verifyContent);

        if (crc_ccitt.length != verifyCode.length){
            return false;
        }

        return Arrays.equals(verifyCode, crc_ccitt);
    }
    ...
    public byte[] generateVerifyCode(byte[] content) {
        ...
        crc_bytes[0] = (byte)((crc_reg >> 8) & 0xff);
        crc_bytes[1] = (byte)((crc_reg >> 0) & 0xff);
    ...
    public VerifyType verifyType() {
        return VerifyType.CRCCCITT;
    }

//    public static void main(String[] argc) throws NoSuchAlgorithmException {
//        byte[] testData = new byte[]{(byte)0x98, 0x00, 0x12, 0x01, 0x00, 0x01, 0x12, 0x34, 0x56, 0x78, 0x5A, 0x26, (byte)0xF9, (byte)0xCE, 0x23};
//
//        IVerify iVerify = new CRC16CCITTVerify();
//
//        System.out.println(GeneraUtil.byte2HexStr(iVerify.generateVerifyCode(testData)));
//    }
}
