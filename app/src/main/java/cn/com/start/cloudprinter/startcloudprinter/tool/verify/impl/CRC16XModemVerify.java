...
public class CRC16XModemVerify implements IVerify {

    private static final char CRC16TABLE[] = { 0x0000, 0x1021, 0x2042, 0x3063,
    ...
    @Override
    public boolean verifyContent(byte[] verifyCode, byte[] verifyContent) {

        byte[] contentCode = this.generateVerifyCode(verifyContent);

        if (verifyCode.length != contentCode.length){
            return false;
        }

        for (int i = 0; i < contentCode.length; ++i){
            if (verifyCode[i] != contentCode[i]){
                return false;
            }
        }
